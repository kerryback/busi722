...
\begin{itemize}
\item $r_i$ = return on asset $i$, \; $r_f$ = risk-free rate, \; $r_m$ = market return
\item $\beta_i = \dfrac{\text{Cov}(r_i, r_m)}{\text{Var}(r_m)}$
\item Higher beta $\Rightarrow$ higher expected return
\end{itemize}
\end{frame}
...
\item Small-stock portfolios earn positive CAPM alphas; large-stock portfolios earn negative alphas
...
\item High B/M (``value'') stocks earn positive CAPM alphas; low B/M (``growth'') stocks earn negative alphas
...
\begin{frame}{Setup Instructions}
Follow the setup guide for your platform:

\begin{itemize}
\item \textbf{Mac:} \href{https://busi722.kerryback.com/files/setup-mac.md}{setup-mac.md}
\item \textbf{Windows:} \href{https://busi722.kerryback.com/files/setup-windows.md}{setup-windows.md}
\item \textbf{Video walkthrough:} \href{https://www.youtube.com/watch?v=hpMrTabldEY}{YouTube}
\end{itemize}

\vspace{0.3cm}
The guides cover Claude Pro, Claude Code, VS Code, Python, and the Rice Data Portal.  Claude Code has both a \alert{native installer} (recommended, auto-updates) and an \alert{npm installer} (deprecated but functional).
\end{frame}
...
Download and configure VS Code for the course:
\begin{itemize}
\item Install VS Code from \href{https://code.visualstudio.com}{code.visualstudio.com}, then File $\rightarrow$ Open Folder $\rightarrow$ your course folder
...
\item Web UI: \href{https://data-portal.rice-business.org}{data-portal.rice-business.org}
\item The data portal is an AI agent that uses ChatGPT to generate SQL and return results in your browser
\end{itemize}

\vspace{0.3cm}
When you visit, you will be given an \alert{access token}.  Save it --- you will need it later for querying the database from Claude Code.
\end{frame}
...
\begin{frame}{Using the Web Portal}
Type natural language queries into the data portal.  It generates SQL and returns results in your browser.

\vspace{0.3cm}
\textbf{Example queries:}
\begin{itemize}
\item ``Show me the closing prices for AAPL in 2024.''
\item ``What are the largest stocks by market cap this month?''
\item ``List all sectors and the number of stocks in each.''
\end{itemize}

\vspace{0.3cm}
The web portal is a good way to explore the data before writing code.
\end{frame}
...
\item \textbf{Signals}: MA Ratio ($\texttt{close} / \text{MA}_{12}$, uptrend if $>1$) and Golden Cross ($\text{MA}_3 > \text{MA}_{12}$).
\end{itemize}
...
\begin{itemize}
\item The first \texttt{groupby} computes the equal-weighted portfolio return each month; the second averages across months.
