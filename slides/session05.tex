...
\vspace{0.3cm}
\begin{shadedbox}[title=The Question]
How do we translate these predictions into portfolio weights?
\end{shadedbox}
...
\vspace{0.3cm}
\begin{shadedbox}[title=Key Observation]
Sorting and equal-weighting within groups is just \textbf{one particular} monotone function mapping predictions to weights.  There are many others.
\end{shadedbox}
\end{frame}
...
\begin{baritemize}
\item Stocks above the median get positive weights; below get negative.
\item Dollar-neutral by construction (weights sum to zero).
...
\begin{baritemize}
\item $\lambda > 0$: tilts toward top-ranked stocks.
\item Larger $\lambda$: more aggressive concentration.
...
\vspace{0.3cm}
\begin{baritemize}
\item Stays close to the investable market-cap benchmark.
\item Large stocks retain significant weight even with low ranks.
...
\begin{baritemize}
\item $\bar r_p$ = mean monthly portfolio return.
...
\item A portfolio can have a high Sharpe ratio but zero alpha (if it just takes on market risk).
\item A portfolio can have a high IR but a low Sharpe ratio (if alpha is consistent but small).
\end{baritemize}
\end{frame}

\begin{frame}{Putting It Together}
\begin{barenumerate}
\item Generate one-step-ahead predicted ranks (Session 4).
\item Choose a weight function: sort-based, linear, power, exponential, softmax, or score-tilted.
\item Compute monthly portfolio returns.
\item Evaluate: Sharpe ratio, CAPM alpha (with $t$-stat), and information ratio.
\item In Session 6: extend evaluation to Fama-French factors and other considerations.
\end{barenumerate}
\end{frame}
