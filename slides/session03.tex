...
\item Outliers in financial data can be extreme --- a single stock with roe $= 50$ can dominate the fit.
...
\item Robust to skewed distributions (e.g., market cap, book-to-market).
\item Can also compute z-scores of ranks for a hybrid approach.
\end{baritemize}
\end{frame}
...
\begin{shadedbox}[title=The Key Insight]
If every stock's return rises by 2\% because the market rallies, that tells us nothing about which stocks to buy.  What matters is the \textbf{cross-sectional ranking} of returns.
\end{shadedbox}
...
\item Removes the influence of market-wide shocks (bull/bear months).
\item Reduces the impact of return outliers (e.g., a stock that doubles in a month).
...
\item Robust to outliers and requires little tuning.
\item Hard to overfit by adding more trees.
\end{itemize}
\end{frame}
...
\item Add the new tree's prediction (scaled by a learning rate $\eta$) to the running total.
\item Repeat for $B$ rounds.
\end{barenumerate}
...
\item Uses \alert{histogram-based} splitting: bins continuous features into discrete buckets for faster split finding.
\item \alert{Leaf-wise} growth (vs.\ level-wise): grows the leaf with the largest loss reduction, producing deeper, more accurate trees.
\item Handles categorical features natively.
...
\item This approximates a \alert{kernel} method (RBF kernel) but scales to large datasets.
\item The bandwidth parameter $\gamma$ controls the ``wiggliness'' of the fit.
\end{baritemize}
...
\begin{baritemize}
\item $\mathbf{W}_1$ is an $h \times k$ weight matrix, $\mathbf{b}_1$ is a bias vector.
\item $\sigma(\cdot)$ is a nonlinear activation: ReLU, tanh, etc.
...
\item Process data in \alert{mini-batches}, updating weights after each batch.
\item One pass through the full training set = one \alert{epoch}.
