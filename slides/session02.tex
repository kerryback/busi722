...
\title{Querying with Claude Code, Fundamental Indicators}
...
\begin{frame}{Installing the Data Skills}
Download \href{https://busi722.kerryback.com/files/rice-data-skills.zip}{rice-data-skills.zip} and extract it to your \texttt{\textasciitilde/.claude/skills/} folder.
\vspace{0.3cm}

This installs four skills that Claude uses to fetch and merge data:
\begin{itemize}
\item \textbf{rice-data-query} -- database connection, schema, SQL patterns
\item \textbf{fetch-returns} -- monthly prices, returns, momentum, size
\item \textbf{fetch-fundamentals} -- financial ratios, growth rates
\item \textbf{merge-data} -- join returns with fundamentals, avoid look-ahead bias
\end{itemize}
\end{frame}

%%% PART 2: QUERYING WITH CLAUDE CODE %%%

\begin{frame}[c]
\centering
\Huge Querying with Claude Code
\end{frame}
...
The \href{https://data-portal.rice-business.org}{Rice Data Portal} provides daily prices, valuation metrics, and fundamental data from SEC filings. Four main tables:
...
\begin{frame}{Querying the Database}
...
\begin{enumerate}
\item Queries \textbf{SEP} for end-of-month prices, \textbf{DAILY} for market cap, and \textbf{TICKERS} for sector/industry
...
\item Uses split- and dividend-adjusted prices (\texttt{closeadj}); requires 13 months of history.
...
\midrule
\texttt{ticker}, \texttt{month} & Identifiers \\
\texttt{return} & Monthly return (decimal) \\
\texttt{momentum} & 12-month return skipping the most recent month \\
\texttt{lagged\_return} & Prior month's return \\
\texttt{close} & Split-adjusted price \\
\texttt{marketcap} & Market cap in thousands (\alert{shifted by 1 month}) \\
\texttt{size} & Nano- through Mega-Cap \\
\texttt{sector}, \texttt{industry} & Classifications from TICKERS table \\
\bottomrule
\end{tabular}
\end{center}
\end{frame}
...
\alert{Prompt:} ``Get annual fundamentals from SF1: equity, assets, gp, and ratios roe, grossmargin, assetturnover, de.  Calculate asset growth and gp/assets.  Save as \texttt{fundamentals.parquet}.''
\vspace{0.3cm}

Claude uses the \texttt{fetch-fundamentals} skill, which:
\begin{enumerate}
\item Checks which variables are precomputed, then queries SF1 for annual data (\texttt{dimension='ARY'})
\item Calculates growth rates \alert{before} merging (critical!)
\item Queries DAILY for valuation ratios if requested and shifts them by 1 month
\end{enumerate}
\end{frame}
...
\textbf{The Problem:} If you calculate \texttt{pct\_change()} after forward-fill, consecutive months with the same value produce \textbf{zero growth} --- which is wrong.
...
\begin{enumerate}
\item Aligns fundamentals to the first month \alert{after} the SEC filing date, then merges on (ticker, month)
\item Forward-fills fundamentals within each ticker, then shifts SF1 variables and \texttt{close} by 1 month
...
\textbf{Look-Ahead Bias:} Using information that was \textbf{not yet available} at the time of the investment decision. Backtests that suffer from this overstate performance --- sometimes dramatically.
...
\begin{itemize}
\item \textbf{Market cap, DAILY ratios, and close price}: all shifted by 1 month so January's values come from end of December.
\item \textbf{SF1 fundamentals} (roe, de, etc.): aligned to the first full month \emph{after} the SEC filing date, then forward-filled.  No additional shift needed.
\item \textbf{Momentum and lagged return}: built with a lag by construction (momentum uses $t{-}13$ to $t{-}2$; lagged return uses $t{-}1$).
\end{itemize}
\end{frame}
...
roe, de, grossmargin, \ldots & SF1 & filing-date alignment only \\
...
\item Common threshold: \$5.00 (our default filter).
\item Example: Drop rows below a price threshold, sort into quintiles on momentum and lagged return, and compute average returns.
\end{itemize}
\end{frame}
