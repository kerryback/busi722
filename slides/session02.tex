...
\begin{itemize}
\item \texttt{ticker}, \texttt{month} (identifiers); \texttt{return} (monthly, decimal); \texttt{momentum} (12-month skipping recent); \texttt{lagged\_return} (prior month)
\item \texttt{close} (split-adjusted price); \texttt{marketcap} (in thousands, \alert{shifted by 1 month}); \texttt{size} (Nano- through Mega-Cap)
\item \texttt{sector}, \texttt{industry} -- classifications from TICKERS table
\end{itemize}
\end{frame}
